...
class Price extends MigratableSeeder
{
    ...
    public function run(): void
    {
        ...
        DB::connection($typeConnection)->table(Configuration::TABLE)->insert([
            ...
            'name_identify' => EnumConfiguration::RD_URI_API_PRICE_PRODUCT,
            'name' => 'URI da API para consultar preço do produto',
            'value' => '',
        ...
        DB::connection($typeConnection)->table(Configuration::TABLE)->insert([
            'name_group' => EnumConfiguration::MS_GROUP,
            'name_identify' => EnumConfiguration::ACTIVATE_LOG_API_PRICE,
            'name' => 'Ativar log da API price',
            'value' => 0,
            'name_store' => EnumStore::ALL_STORES,
            'type_configuration' => EnumTypeConfiguration::STRING,
            'channel_site' => 1,
    ...
    public function down(): void
    {
        ...
        DB::connection($typeConnection)->table(Configuration::TABLE)
            ...
            ->where('name_identify', EnumConfiguration::RD_URI_API_PRICE_PRODUCT)
            ->delete();

        DB::connection($typeConnection)->table(Configuration::TABLE)
            ->where('name_group', EnumConfiguration::MS_GROUP)
            ->where('name_identify', EnumConfiguration::ACTIVATE_LOG_API_PRICE)
            ->delete();
    }
}
